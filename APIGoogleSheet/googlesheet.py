...
class GoogleSheet:
    ...
    def shift_orders_header(self, today):
        ...
        # Сохраняем формулы из столбцов, которые не попадают в диапазон смещения
        formulas_to_preserve = df_formulas.iloc[:, 133:].values

        # Смещение заголовков и содержимого столбцов
        header_values = df_values.columns[103:133].tolist()  # Индексы столбцов
        shifted_header_values = header_values[:29]
        shifted_header_values.insert(0, today)
        # Обновление заголовков
        df_values.columns = df_values.columns[:103].tolist() + shifted_header_values + df_values.columns[133:].tolist()
        df_formulas.columns = df_values.columns  # Обновляем заголовки в формулах
        # Смещение содержимого столбцов
        df_values.iloc[:, 104:133] = df_values.iloc[:, 103:132].values
        df_values.iloc[:, 103] = ""  # Очистка первого столбца

        # Восстанавливаем формулы в столбцах, которые не попадают в диапазон смещения
        df_formulas.iloc[:, 104:133] = df_formulas.iloc[:, 103:132].values
        df_formulas.iloc[:, 103] = ""  # Очистка первого столбца
        df_formulas.iloc[:, 133:] = formulas_to_preserve
