        ...
        if len(edit_data_from_table) > 0:
            ...
            for account, nm_ids_data in edit_data_from_table.items():
                ...
                if len(valid_data_result) > 0:
                    ...
                    edit_nm_ids_data = service_gs_table.change_cards_and_tables_data(valid_data=valid_data_result)
                    updates_nm_ids_data.update(edit_nm_ids_data)
            if len(updates_nm_ids_data) > 0:
                gs_connection().update_rows(data_json=updates_nm_ids_data,
...
schedule.every(35).seconds.do(main)
