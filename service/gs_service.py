...
class ServiceGoogleSheet:
    ...
    def add_new_data_from_table(self, nm_ids, edit_column_clean=None, only_edits_data=False):
    ...
    def change_cards_and_tables_data(self, valid_data):
        ...
        self.add_new_data_from_table(nm_ids_result, edit_column_clean=edit_column_clean, only_edits_data=True)

        # todo исправить избыточное обращение к АПИ, если по данным не было изменений
        """вроде исправил"""

    # def edit_last_dynamic_data_for_nm_ids(self, nm_ids):
    #
    #     card_from_nm_ids_filter = self.wb_api_content.get_list_of_cards(nm_ids_list=nm_ids, limit=100,
    ...
    def add_new_day_revenue_to_table(self):
        ...
        if self.gs_service_revenue_connect.check_last_day_header_from_table(last_day=last_day):
            ...
            for account, articles in lk_articles.items():
                ...
                self.gs_service_revenue_connect.add_last_day_revenue(nm_ids_revenue_data=revenue_data_by_article,
                                                                     last_day=last_day)
                print(f"добавлена выручка по новом заголовку {last_day} по всем артикулам")

    def add_actually_data_to_table(self):
        print("актуализируем данные в таблице")
        """
